...
A machine learning project that predicts whether a patient is at risk of heart disease using clinical and demographic features. Four classification models are compared, and the most reliable one is recommended for screening use. The final model is deployed as a live, interactive web application.
...
12. **Deployed the final model as a live web app**
...
- **Gradio** — used to build a clean, user-friendly interface with sliders, dropdowns, and radio buttons for each clinical input. Users enter values in plain English (e.g. "Typical Angina") instead of numeric codes.
- **Hugging Face Spaces** — used as the free hosting platform, making the model accessible to anyone with a browser, no setup required.
- **Joblib** — used to serialize (save) the trained XGBoost model and the StandardScaler so they can be loaded by the deployed app.

The deployed interface applies the same preprocessing pipeline used during training, then returns:
- A binary risk verdict (High Risk / Low Risk)
...
- `xgb_heart_model.pkl` — Serialized trained XGBoost model
...
4. To run the Gradio app locally:
   ```bash
...
- **Deployment:** Gradio, Hugging Face Spaces, Joblib
